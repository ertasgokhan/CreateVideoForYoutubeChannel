...
namespace CreateVideoForYoutubeChannel.Controllers
{
    public class TabletController : Controller
    {
        ...
        public IActionResult Index(YouTubeModel model)
        {
            ...
            string filepath = @"C:\Yutup\TABLET\" + model.ProductName.Replace(" ", "").Replace("/", "").Trim() + ".txt";
                    ...
                    if (groups != null && groups.Count > 0)
                    {
                        ...
                        for (int i = 0; i < groups.Count; i++)
                        {
                            ...
                            foreach (var item2 in groupChilNodes)
                            {
                                ...
                                if (item2.Name == "li")
                                {
                                    propertyName = item2.SelectSingleNode("strong").InnerText.Trim();
        ...
        public IActionResult TabletVS()
        {
            return View();
        }

        [HttpPost]
        public IActionResult TabletVS(TabletVSModel tabletModel)
        {
            // PPT

            List<string> tablet1Properties = new List<string>();
            List<string> tablet2Properties = new List<string>();

            string filepathPhone1 = @"C:\Yutup\TABLET\" + tabletModel.Tablet1.Replace(" ", "") + ".txt";
            string filepathPhone2 = @"C:\Yutup\TABLET\" + tabletModel.Tablet2.Replace(" ", "") + ".txt";
            ...
            using (StreamReader rd = System.IO.File.OpenText(filepathPhone2))
            {
                ...
                {
                    string str = rd.ReadLine();
                    tablet2Properties.Add(str);
                }
            }

            // just gets me the current location of the assembly to get a full path
            string fileName = @"C:\Yutup\TABLET\VS\VS_PPT_TABLET.pptx";
            ...
            using (PresentationDocument document = PresentationDocument.Open(fileName, true))
            {
                ...
                foreach (var item in tablet1Properties)
                {
                    ...
                    foreach (SlidePart slidePart in document.PresentationPart.SlideParts)
                    {
                        ...
                        // change the text
                        if (text != null)
                            text.Text = tablet1Properties.FirstOrDefault(x => x.Contains(item)).Split("->")[1].Replace("Var", "Yes").Replace("Yok", "No").Replace("Milyon", "Million").Replace("Milyar", "Billion").Replace("Cam", "Glass").Replace("Alüminyum", "Aluminum").Replace("Gram", "Grams").Replace("Çekirdek", "Core").Replace("Kablosu", "Cable").Replace("Piksel", "Pixel").Replace("Çift Hat", "Dual SIM").Replace("Ocak", "January").Replace("Şubat", "February").Replace("Mart", "March").Replace("Nisan", "April").Replace("Mayıs", "May").Replace("Haziran", "June").Replace("Temmuz", "July").Replace("Ağustos", "August").Replace("Eylül", "September").Replace("Ekim", "October").Replace("Kasım", "November").Replace("Aralık", "December").Replace("Robot Süpürge", "Robot Vacuum").Replace("Sert Zemin", "Hard Floor").Replace("Halı", "Carpet").Replace("saat", "hour(s)").Replace("dk", "min").Replace("Süpürme", "Vacuum").Replace("Mobil Uygulama", "Mobile Application").Replace("Adet", "piece(s)").Replace("Siyah", "Black").Replace("Beyaz", "White").Replace("Mobil Uygulama", "Mobile Application").Replace("Google Asistan", "Google Assistant").Replace("Lazer", "Laser").Replace("MobilUygulamaİle", "By Mobile Application").Replace("Otomatik", "Automatic").Replace("Manyetik Bant", "Magnetic Tape").Replace("Jiroskop", "Gyroscope").Replace("Kamera", "Camera").Replace("Algoritma", "Algorithm").Replace("SIM Destekli", "SIM Supported").Replace("Nesil", "Generation").Replace("İnç", "Inch").Replace("Retina Ekran", "Retina Display").Replace("Çip", "Chip").Replace("Mikro Lokasyon", "Micro Location").Replace("Azami", "Maximum").Replace("Saat", "Hour(s)").Replace("Gri", "Grey").Replace("Gümüş", "Silver").Replace("Koyu", "Dark").Replace("Hızlı Şarj", "Fast Charging");
                    }
                }

                foreach (var item in tablet2Properties)
                {
                    ...
                    foreach (SlidePart slidePart in document.PresentationPart.SlideParts)
                    {
                        ...
                        // change the text
                        if (text != null)
                            text.Text = tablet2Properties.FirstOrDefault(x => x.Contains(item)).Split("->")[1].Replace("Var", "Yes").Replace("Yok", "No").Replace("Milyon", "Million").Replace("Milyar", "Billion").Replace("Cam", "Glass").Replace("Alüminyum", "Aluminum").Replace("Gram", "Grams").Replace("Çekirdek", "Core").Replace("Kablosu", "Cable").Replace("Piksel", "Pixel").Replace("Çift Hat", "Dual SIM").Replace("Ocak", "January").Replace("Şubat", "February").Replace("Mart", "March").Replace("Nisan", "April").Replace("Mayıs", "May").Replace("Haziran", "June").Replace("Temmuz", "July").Replace("Ağustos", "August").Replace("Eylül", "September").Replace("Ekim", "October").Replace("Kasım", "November").Replace("Aralık", "December").Replace("Robot Süpürge", "Robot Vacuum").Replace("Sert Zemin", "Hard Floor").Replace("Halı", "Carpet").Replace("saat", "hour(s)").Replace("dk", "min").Replace("Süpürme", "Vacuum").Replace("Mobil Uygulama", "Mobile Application").Replace("Adet", "piece(s)").Replace("Siyah", "Black").Replace("Beyaz", "White").Replace("Mobil Uygulama", "Mobile Application").Replace("Google Asistan", "Google Assistant").Replace("Lazer", "Laser").Replace("MobilUygulamaİle", "By Mobile Application").Replace("Otomatik", "Automatic").Replace("Manyetik Bant", "Magnetic Tape").Replace("Jiroskop", "Gyroscope").Replace("Kamera", "Camera").Replace("Algoritma", "Algorithm").Replace("SIM Destekli", "SIM Supported").Replace("Nesil", "Generation").Replace("İnç", "Inch").Replace("Retina Ekran", "Retina Display").Replace("Çip", "Chip").Replace("Mikro Lokasyon", "Micro Location").Replace("Azami", "Maximum").Replace("Saat", "Hour(s)").Replace("Gri", "Grey").Replace("Gümüş", "Silver").Replace("Koyu", "Dark").Replace("Hızlı Şarj", "Fast Charging");
                    }
                }
            ...
            const string srcFileName = @"C:\Yutup\TABLET\VS\VS_PPT_TABLET.pptx";
            ...
            for (int i = 0; i < nbSlides; i++)
            {
                ...
                slide.ReplacePicture("product1", @"C:\Yutup\TABLET\VS\product1.png", "image/png");
                slide.ReplacePicture("product2", @"C:\Yutup\TABLET\VS\product2.png", "image/png");
